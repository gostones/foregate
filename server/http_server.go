...
func (h *HTTPServer) GoListenAndServe(addr string, handler http.Handler) error {
	...
	//ping
	r.HandleFunc("/__/ping", ping)

	//apis

	//tunnel
	r.Handle("/__/tunnel", handler)

	//web proxy
	r.HandleFunc("/__/{port}", func(res http.ResponseWriter, req *http.Request) {
		...
		log.Printf("Hanle /__/port: %v %v  url: %v", port, s, req.URL)
